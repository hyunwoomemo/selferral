...
export function useEmployeeTableFilters() {
  const [exchangeQuery, setExchangeQuery] = useQueryState("exchange", searchParams.exchange.withOptions({ shallow: false, throttleMs: 500 }).withDefault("0"));

  const [orderQuery, setOrderQuery] = useQueryState("order", searchParams.order.withOptions({ shallow: false, throttleMs: 500 }).withDefault("createtime"));

  const [orderbyQuery, setOrderbyQuery] = useQueryState("orderby", searchParams.orderBy.withOptions({ shallow: false, throttleMs: 500 }).withDefault("desc"));
